...
test_own_cmd_success "gpg create from input" echo "$GPG_KEY_CONTENT" | $CMD gpg_key create --name="$GPG_KEY_NAME_2" --org="$TEST_ORG"
...
test_own_cmd_success "gpg update from input" echo "$GPG_KEY_CONTENT" | $CMD gpg_key update --name="$GPG_KEY_NAME" --org="$TEST_ORG" --new_content
